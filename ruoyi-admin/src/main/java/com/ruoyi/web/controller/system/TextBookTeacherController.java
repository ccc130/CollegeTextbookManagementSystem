...
public class TextBookTeacherController extends BaseController
{
    @Autowired
    ...
    {
        startPage();
        List<SysUser> list = userService.selectUsersByRoleId(user, 101L);
    ...
    @PreAuthorize("@ss.hasPermi('system:teacher:import')")
     ...
     * 重置密码
     */
    @PreAuthorize("@ss.hasPermi('system:teacher:resetPwd')")
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @PutMapping("/resetPwd")
